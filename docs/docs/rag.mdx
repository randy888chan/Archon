...
| **LLM_PROVIDER** | Provider (openai/google/openrouter/ollama) | `openai` | Model availability |
...
EMBEDDING_MODEL=text-embedding-3-small
# Pros: Access to 200+ models, competitive pricing
# Cons: Requires separate embedding provider
```

<Admonition type="info" title="OpenRouter + Embeddings">
OpenRouter provides access to chat models from multiple providers (OpenAI, Anthropic, Meta, Google, etc.) but doesn't offer embedding models. For embeddings, use a separate provider like OpenAI, Google, or Ollama.

**Recommended Setup:**
- **LLM Provider**: OpenRouter (for chat models)
- **Embedding Provider**: OpenAI (for embeddings)
- **API Keys**: Both OpenRouter and OpenAI keys required
</Admonition>
