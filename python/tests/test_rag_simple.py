...
class TestHybridSearchCore:
    ...
    def test_merge_results_functionality(self, hybrid_strategy):
        ...
        vector_results = [
            {'id': '1', 'content': 'Vector result', 'similarity': 0.9,
             'url': 'test1.com', 'chunk_number': 1, 'metadata': {}, 'source_id': 'src1'}
        ]
        keyword_results = [
            {'id': '2', 'content': 'Keyword result', 'url': 'test2.com',
             'chunk_number': 1, 'metadata': {}, 'source_id': 'src2'}
        ]
...
class TestRAGIntegrationSimple:
    ...
    async def test_code_search_with_agentic_rag(self, rag_service):
        ...
        with patch.object(rag_service.agentic_strategy, 'is_enabled') as mock_enabled, \
            ...
            success, result = await rag_service.search_code_examples_service(
                query="python greeting function",
                match_count=10
            )
