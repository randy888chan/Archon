...
def calculate_similarity(str1: str, str2: str, max_chunk: int = 100) -> float:
    """
    ...
    Args:
        ...
        max_chunk: Maximum number of characters to compare

    Returns:
        float: Similarity score between 0 and 1
    """
    comp_len = min(len(str1), len(str2), max_chunk)
    str1_chunk = str1[:comp_len]
    str2_chunk = str2[:comp_len]

    return SequenceMatcher(None, str1_chunk, str2_chunk).ratio()


def find_best_match(truncated: str,
                    full_items: List[Dict[str, str]],
                    max_chunk: int = 100,
                    similarity_threshold: float = 0.8) -> Tuple[int, Dict[str, str], float]:
    """
    Find the best matching item from full_items that matches the truncated string.

    Args:
        truncated: The truncated string to match
        full_items: List of dictionaries containing 'content' and 'url'
        max_chunk: Maximum number of characters to compare
        similarity_threshold: Minimum similarity score to consider a match

    Returns:
        Tuple[int, Dict[str, str], float]: Index, matching dictionary item, and similarity score
    """
    best_match = (-1, {}, 0.0)

    for idx, item in enumerate(full_items):
        similarity = calculate_similarity(
            truncated, item['content'], max_chunk)

        if similarity > best_match[2]:
            best_match = (idx, item, similarity)
...
def reorder_by_content(content_one: List[str],
                       content_two: List[Dict[str, str]],
                       max_chunk: int = 100,
                       similarity_threshold: float = 0.8) -> List[Dict[str, str]]:
    """
    Reorders content_two based on matching content with content_one.

    Args:
        content_one: List of truncated strings in the correct order
        content_two: List of dictionaries containing 'content' and 'url' pairs
        max_chunk: Maximum number of characters to compare for matching
        similarity_threshold: Minimum similarity score to consider a match

    Returns:
        List[Dict[str, str]]: Reordered list of dictionaries maintaining the content-url pairs

    Raises:
        ValueError: If either input list is empty
        RuntimeError: If critical matching errors occur
    """
    if not content_one or not content_two:
        raise ValueError("Both input lists must contain elements")

    write_to_log(f"Starting reordering process with {len(content_one)} source strings "
                 f"and {len(content_two)} target items")

    # Create a working copy of content_two that we can modify
    remaining_items = content_two.copy()
    result = [{}] * len(content_two)
    matches_found = 0

    # For each truncated string, find its match and place it in the correct position
    for new_idx, trunc_str in enumerate(content_one):
        match_idx, matched_item, similarity = find_best_match(
            trunc_str, remaining_items, max_chunk, similarity_threshold
        )

        if match_idx != -1:
            # Place the matched item in the position corresponding to the truncated string
            result[new_idx] = matched_item
            # Remove the matched item from remaining items
            remaining_items.pop(match_idx)
            matches_found += 1
            write_to_log(f"Matched '{trunc_str}' with '{matched_item['content']}' "
                         f"(similarity: {similarity:.2f})")
        else:
            write_to_log(f"No match found for truncated string: '{trunc_str}'")

    write_to_log(
        f"Found {matches_found} matches out of {len(content_one)} strings")

    # Add any remaining unmatched items at the end
    if remaining_items:
        write_to_log(f"Processing {len(remaining_items)} unmatched items")
        # Fill in any empty spots in the result with remaining items
        empty_indices = [i for i, x in enumerate(result) if not x]
        for i, remaining_item in enumerate(remaining_items):
            if i < len(empty_indices):
                result[empty_indices[i]] = remaining_item
            else:
                result.append(remaining_item)

    # Remove any empty dictionaries that might remain
    result = [item for item in result if item]

    if not result:
        raise RuntimeError("Failed to produce any valid matches")

    write_to_log(
        f"Reordering complete. Final list contains {len(result)} items")
    return result
    ...
    if os.path.exists(env_file_path):
        try:
            with open(env_file_path, "r") as f:
                ...
                current_profile = profile or env_vars.get(
    ...
    if os.path.exists(env_file_path):
        try:
            with open(env_file_path, "r") as f:
                ...
                # If profile is specified, use it; otherwise use current profile
                current_profile = profile_name or env_vars.get(
                    "current_profile", "default")
...
def reload_archon_graph(show_reload_success=True):
    ...
    try:
        # First reload agent coders
        import archon.pydantic_ai_coder
        importlib.reload(archon.pydantic_ai_coder)

        import archon.langchain_js_coder
        importlib.reload(archon.langchain_js_coder)

        import archon.langchain_python_coder
        importlib.reload(archon.langchain_python_coder)

        import archon.langgraph_coder
        importlib.reload(archon.langgraph_coder)

        import archon.langgraphjs_coder
        importlib.reload(archon.langgraphjs_coder)

        import archon.langsmith_coder
        importlib.reload(archon.langsmith_coder)

        # Then reload archon_graph which imports agent coders
        import archon.archon_graph
        importlib.reload(archon.archon_graph)

        # Then reload the crawler
        import archon.crawl_ai_docs
        importlib.reload(archon.crawl_ai_docs)

        if show_reload_success:
            st.success(
                "Successfully reloaded Archon modules with new environment variables!")
        return True
    except Exception as e:
