...
class HierarchicalChunker:
    ...
    def _process_node(self, node, ancestor_path, chunks):
        ...
        if node_content or content_type in ["link_list", "code_example", "table_based"]: # Process if content exists or specific types
            if content_type == "link_list":
                ...
                chunks.append(self._create_chunk(node, current_path, "link_list"))
                 ...
                 if "code_blocks" in node and isinstance(node["code_blocks"], list):
                     ...
                     for code_block_node in node["code_blocks"]:
                         ...
                         chunks.append(self._create_chunk(code_block_node, current_path, "code_example"))
                ...
                self._chunk_descriptive_content(node, current_path, chunks)
                 ...
                 if node.get("type") == "document" or node.get("metadata", {}).get("section_type") == "root":
            ...
            elif node_content: # Handle nodes with content but unclassified or 'default' type
    ...
    def _create_chunk(self, node, path, chunk_type):
        ...
        metadata = node.get("metadata", {}).copy() # Use copy to avoid modifying original node
        ...
        return {
            ...
            "metadata": {
                ...
                "content_type": metadata.get("content_type"), # Use the determined chunk_type
                "header_level": node.get("level", 0), # Header level of the node
                # Add other relevant metadata from the node if needed
                "language": node.get("language") if chunk_type == "code_example" else None,
            }
        }
    ...
    def _chunk_descriptive_content(self, node, path, chunks):
        ...
        for i, paragraph in enumerate(paragraphs):
            ...
            potential_len = len(current_chunk_content) + len(paragraph) + (2 if current_chunk_content else 0) # +2 for '\n\n'
            ...
            if potential_len <= self.max_chunk_size:
                ...
                if current_chunk_content:
                    ...
                    chunks.append(self._create_chunk(chunk_node, path, "descriptive_text_oversized")) # Mark as oversized
    ...
    def add_hierarchical_context(self, chunks):
        ...
        for chunk in chunks:
            ...
            header_level = chunk.get("metadata", {}).get("header_level", 0) # Use level from metadata
            ...
            if chunk_title:
                ...
                pattern = r"^\s*(#{{1,{level}}}\s*{title}|{title}\s*\n[-=]+)\s*\n".format(
            ...
            if "metadata" not in enhanced_chunk: enhanced_chunk["metadata"] = {}
    ...
    def establish_cross_references(self, chunks):
        ...
        for i, chunk in enumerate(chunks):
            ...
            # Ensure metadata exists
            if "metadata" not in chunks[i]: chunks[i]["metadata"] = {}
            ...
            chunks[i]["metadata"]["related_sections_info"] = related_sections_info # Store richer info
...
if __name__ == '__main__':
    # This requires a sample document_tree structure
    # For testing, you would typically load a pre-processed tree from a JSON file
    # or run the MarkdownProcessor first.
    print("Chunker script run directly.")
    print("To test, provide a sample document_tree dictionary.")

    # Placeholder example structure:
    sample_tree = {
        "type": "document", "title": "Sample Doc", "content": "Introduction text.",
        "metadata": {"section_type": "root", "content_type": "introduction"},
        "children": [
            {"type": "header", "title": "Section 1", "level": 1, "content": "Content for section 1. More descriptive text.",
             "metadata": {"section_type": "General", "content_type": "descriptive_text"}, "children": []},
            {"type": "header", "title": "Section 2", "level": 1, "content": "",
             "metadata": {"section_type": "General", "content_type": None},
             "children": [
                 {"type": "header", "title": "Subsection 2.1", "level": 2, "content": "```python\nprint('Hello')\n```",
                  "metadata": {"section_type": "Examples", "content_type": "code_example"}, "children": []}
             ]}
        ]
    }

    print("\nProcessing sample tree...")
    final_chunks = process_chunks(sample_tree)
    print(f"\nGenerated {len(final_chunks)} final chunks from sample.")

    # Optionally print the chunks
    # print("\nSample Chunks Output:")
    # print(json.dumps(final_chunks, indent=2))