...
class OpenAIEmbeddingGenerator:
    ...
    def __init__(self, env_loader: Optional[EnvironmentLoader] = None):
        ...
        print(f"Using OpenAI embedding model: {self.embedding_model}", flush=True)
    ...
    def generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            print(f"OpenAI API error generating single embedding: {e}", flush=True)
            raise  # Re-raise the specific API error
        except Exception as e:
            print(f"Unexpected error generating single embedding: {e}", flush=True)
    ...
    def generate_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        for i in range(0, len(processed_texts), batch_size):
            ...
            try:
                print(
                    f"Generating embeddings for batch {batch_index_info} of size {len(batch)}...",
                    flush=True,
                ...
                if response.data and len(response.data) == len(batch):
                    ...
                    print(
                        f"Successfully processed batch {batch_index_info}.", flush=True
                    )  # Added success print
                else:
                    raise ValueError(
