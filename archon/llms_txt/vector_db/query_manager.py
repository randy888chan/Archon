...
class HierarchicalQueryManager:
    ...
    def __init__(
        ...
        _env_loader = env_loader or EnvironmentLoader(env_file_path="../../workbench/env_vars.json")
    ...
    def search(
        ...
        """
        if not query:
            print("Warning: Search query is empty. Returning empty results.")
            return []
        ...
        try:
            ...
            print(f"Semantic search found {len(results)} potential matches.")
            return results

        except Exception as e:
            print(f"Error during semantic search for query '{query[:50]}...': {e}")
    ...
    def hierarchical_search(
        ...
        """
        if not query:
            print("Warning: Hierarchical search query is empty. Returning empty results.")
            return []

        print(f"Performing hierarchical search for query: '{query[:50]}...'")
        try:
            # 1. Perform the base semantic search
            base_results = self.search(query, match_count=match_count)

            if not base_results:
                print("Hierarchical search: Base semantic search returned no results.")
                return []

            print(f"Hierarchical search: Found {len(base_results)} base results. Fetching context...")
            enriched_results = []
            processed_node_ids = set() # Avoid processing context for the same node multiple times if returned by base search
            ...
            for result in base_results:
                ...
                print(f"Fetching context for node ID: {node_id}")
                ...
                main_node_full = next((n for n in context_by_type["self"] if n.get("id") == node_id), result)

                # Build the enriched result structure
                enriched_node = {
                    "main_node": main_node_full, # Use the potentially more detailed node from context
                    "similarity": result.get("similarity"), # Keep original similarity score
                    "parents": sorted(context_by_type["parent"], key=lambda x: x.get("context_level", 0)), # Sort parents by level
                    ...
                    "references": context_by_type["reference"] if include_references else []
                }
                enriched_results.append(enriched_node)

            print(f"Hierarchical search completed. Returning {len(enriched_results)} enriched results.")
            return enriched_results

        except Exception as e:
            print(f"Error during hierarchical search for query '{query[:50]}...': {e}")
            return [] # Return empty list on error
    ...
    def path_based_search(
        ...
        """
        if not path_query:
            print("Warning: Path-based search query is empty. Returning empty results.")
            return []

        print(f"Performing path-based search for pattern: '{path_query}'")
        try:
            ...
            if not path_results:
                print("Path-based search: No nodes found matching the path pattern.")
                return []

            # 2. If no semantic query, return path results directly (limited)
            if not semantic_query:
                print(f"Path-based search: Found {len(path_results)} nodes by path. Returning top {max_results}.")
                return path_results[:max_results]

            # 3. If semantic query provided, re-rank path results
            print(f"Path-based search: Found {len(path_results)} nodes by path. Re-ranking with semantic query: '{semantic_query[:50]}...'")
            ...
            if not node_ids:
                 print("Path-based search: Nodes found by path have no IDs for re-ranking.")
            ...
            id_filter = {"id": node_ids} # Assuming direct list works or RPC handles 'id = ANY(ids_array)'
            ...
            print(f"Path-based search: Re-ranked results yield {len(semantic_results)} nodes.")
            return semantic_results

        except Exception as e:
            print(f"Error during path-based search for pattern '{path_query}': {e}")
            return [] # Return empty list on error