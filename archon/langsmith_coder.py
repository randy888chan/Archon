...
SOURCE_NAME = "langsmith_docs"  # Specific source for this agent
# ---------------------
provider = get_env_var('LLM_PROVIDER') or 'OpenAI'
...
langsmith_coder = Agent(
    model,
    system_prompt=primary_coder_prompt,
    deps_type=LangSmithCoderDeps,  # Use specific Deps class
    retries=2
)
...
@langsmith_coder.system_prompt
# Use specific Deps class
def add_reasoner_output(ctx: RunContext[LangSmithCoderDeps]) -> str:
    # Update prompt to mention LangSmith context if needed, keeping generic for now
    return f"""
...
@langsmith_coder.tool
# Use specific Deps class
async def retrieve_relevant_documentation(ctx: RunContext[LangSmithCoderDeps], user_query: str) -> str:
    """
    Retrieve relevant LangSmith documentation chunks based on the query with RAG.
    ...
    Returns:
        A formatted string containing the top 4 most relevant LangSmith documentation chunks
    """
...
@langsmith_coder.tool
# Use specific Deps class
async def list_documentation_pages(ctx: RunContext[LangSmithCoderDeps]) -> List[str]:
    """
    Retrieve a list of all available LangSmith documentation pages ({SOURCE_NAME}).

    Returns:
        List[str]: List of unique URLs for LangSmith documentation pages
    """
    # Pass the source filter
...
@langsmith_coder.tool
# Use specific Deps class
async def get_page_content(ctx: RunContext[LangSmithCoderDeps], url: str) -> str:
    """
    Retrieve the full content of a specific LangSmith documentation page ({SOURCE_NAME}).
...
langsmith_coder.new_messages_json = get_messages_json
