...
async def start_crawl_with_requests(supabase_client, 
                                    progress_callback=None,
                                    sitemap_url="https://docs.crawl4ai.com/sitemap.xml",
                                    limit_urls=100) -> CrawlProgressTracker:
    """Start the crawling process for Crawl4AI documentation.
    
    Args:
        supabase_client: Initialized Supabase client
        progress_callback: Optional callback function for tracking progress
        sitemap_url: URL of the sitemap to crawl
        limit_urls: Maximum number of URLs to process
        
    Returns:
        CrawlProgressTracker instance with progress information
    """
    progress = CrawlProgressTracker(progress_callback=progress_callback)
    progress.start()
    
    try:
        # First, get the URLs from the sitemap
        progress.log(f"Fetching sitemap from {sitemap_url}")
        ...
        # Find all URL elements in the sitemap (handle different formats)
        urls = []
        namespaces = {'sm': 'http://www.sitemaps.org/schemas/sitemap/0.9'}
        
        # Try to find URL elements with or without namespace
        url_elements = root.findall(".//sm:url", namespaces) or root.findall(".//url")
        
        for url_elem in url_elements:
            # Handle both formats again for the loc element
        ...
        progress.urls_found = len(urls)
        progress.log(f"Found {progress.urls_found} URLs in sitemap")
        
        # Set up crawl4ai config
        ...
        crawler_config = CrawlerRunConfig(
            ...
            wait_for=3000,  # Wait for the page to load
            screenshot=False,
        ...
        async def process_url(url):
            try:
                # First, check if URL was already crawled
                existing_data = await supabase_client.table("site_pages") \
                    .select("id") \
                    .eq("url", url) \
                    .limit(1) \
                    .execute()
                ...
                if existing_data.data:
                    progress.log(f"URL already exists in database, skipping: {url}")
                    progress.urls_processed += 1
                    progress.urls_succeeded += 1
                    return
                
                progress.log(f"Processing URL: {url}")
                ...
                if not crawl_result or not crawl_result.html:
                    progress.log(f"Failed to crawl URL: {url}")
                    progress.urls_processed += 1
                    progress.urls_failed += 1
                    return
                
                # Convert HTML to markdown
                markdown_content = html_converter.handle(crawl_result.html)
                
                # Split into chunks
                chunks = chunk_text(markdown_content)
                
                if not chunks:
                    progress.log(f"No content chunks extracted from URL: {url}")
                    progress.urls_processed += 1
                    progress.urls_failed += 1
                    return
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    progress.chunks_stored += 1
                
                progress.log(f"Successfully processed URL: {url} into {len(chunks)} chunks")
                progress.urls_processed += 1
                progress.urls_succeeded += 1
                
            except Exception as e:
                progress.log(f"Error processing URL {url}: {str(e)}")
                progress.urls_processed += 1
                progress.urls_failed += 1
        ...
        for i in range(0, len(urls), batch_size):
            ...
            if progress_callback:
                progress_callback(progress.get_status())
        
        progress.log(f"Crawl completed. Processed {progress.urls_processed} URLs, stored {progress.chunks_stored} chunks.")
        progress.complete()
        
        return progress
    
    except Exception as e:
        progress.log(f"Error during crawl: {str(e)}")
        progress.is_running = False
        progress.end_time = datetime.now()
        
        # Call the progress callback with the error status
        if progress_callback:
            progress_callback(progress.get_status())
        
        return progress

async def clear_existing_records(supabase_client) -> int:
    """Clear all existing Crawl4AI documentation records from the database.
    ...
    try:
        ...
        result = await supabase_client.table("site_pages") \
            .select("id") \
        ...
        # Delete all records with source=crawl4ai_docs
        await supabase_client.table("site_pages") \
            .delete() \
            .eq("metadata->>source", "crawl4ai_docs") \
            .execute()
