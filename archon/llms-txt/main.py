...
# Assuming Phase 1-3 components are directly in archon/llms-txt/
# Adjust imports if they are structured differently (e.g., inside subdirs)
try:
    from .markdown_processor import MarkdownProcessor
    # Assuming HierarchicalChunker is the correct name based on Phase 2 spec
    from .hierarchical_chunker import HierarchicalChunker
    from .metadata_enricher import MetadataEnricher
except ImportError as e:
     print(f"Error importing Phase 1-3 components: {e}")
     print("Please ensure markdown_processor.py, hierarchical_chunker.py, and metadata_enricher.py exist in archon/llms-txt/")
     raise

# Import Phase 4 components from vector_db subdirectory
try:
    from .vector_db.supabase_manager import SupabaseManager
    from .vector_db.embedding_manager import OpenAIEmbeddingGenerator
    from .vector_db.query_manager import HierarchicalQueryManager
    # Import EnvironmentLoader to potentially pass to managers if needed, though defaults handle it
    from .utils.env_loader import EnvironmentLoader
except ImportError as e:
     print(f"Error importing Phase 4 vector_db components: {e}")
     print("Please ensure supabase_manager.py, embedding_manager.py, and query_manager.py exist in archon/llms-txt/vector_db/")
     raise
...
def process_document(file_path: str, document_id: Optional[str] = None) -> Optional[str]:
    ...
    if not file_path.lower().endswith(".md"):
        print(f"Warning: File {file_path} does not have a .md extension. Attempting to process anyway.")

    # --- Determine Document ID ---
    effective_document_id = document_id or os.path.splitext(os.path.basename(file_path))[0]
    ...
    try:
        ...
        # Ensure class names match the actual implementation files
        chunker = HierarchicalChunker() # Assuming this is the correct class name
        enricher = MetadataEnricher()
        db = SupabaseManager() # Uses default env loader path
    ...
    try:
        ...
        # Assuming enrich_chunks modifies the list of chunks in place or returns a new list
        enriched_chunks = enricher.enrich_chunks(chunks) # Ensure this matches enricher's signature
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process Markdown documents into a hierarchical vector database using Supabase."
    )
    parser.add_argument(
        "--file", "-f",
        required=True,
        help="Path to the single Markdown file to process."
    )
    parser.add_argument(
        "--id",
        ...
        try:
            ...
            for i, result_cluster in enumerate(results):
                ...
                if parents:
                    ...
                    print(f"  Parents: {' -> '.join(parent_paths)}")
                else:
                    print("  Parents: (None)")
