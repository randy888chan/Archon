...
SOURCE_NAME = "langgraph_docs"  # Specific source for this agent
# ---------------------
provider = get_env_var('LLM_PROVIDER') or 'OpenAI'
...
langgraph_coder = Agent(
    model,
    system_prompt=primary_coder_prompt,
    deps_type=LangGraphCoderDeps,  # Use specific Deps class
    retries=2
)
...
langgraph_coder.new_messages_json = get_messages_json

# Update decorator and context type hint


@langgraph_coder.system_prompt
# Use specific Deps class
def add_reasoner_output(ctx: RunContext[LangGraphCoderDeps]) -> str:
...
@langgraph_coder.tool
# Use specific Deps class
async def retrieve_relevant_documentation(ctx: RunContext[LangGraphCoderDeps], user_query: str) -> str:
    """
    Retrieve relevant LangGraph documentation chunks based on the query with RAG.
    ...
    Returns:
        A formatted string containing the top 4 most relevant LangGraph documentation chunks
    """
...
@langgraph_coder.tool
# Use specific Deps class
async def list_documentation_pages(ctx: RunContext[LangGraphCoderDeps]) -> List[str]:
    """
    Retrieve a list of all available LangGraph documentation pages ({SOURCE_NAME}).

    Returns:
        List[str]: List of unique URLs for LangGraph documentation pages
    """
    # Pass the source filter
...
@langgraph_coder.tool
# Use specific Deps class
async def get_page_content(ctx: RunContext[LangGraphCoderDeps], url: str) -> str:
    """
    Retrieve the full content of a specific LangGraph documentation page ({SOURCE_NAME}).
