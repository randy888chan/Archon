...
def database_tab(supabase):
    ...
    vector_dim = st.selectbox(
        ...
        index=0,
        help="Use 1536 for OpenAI embeddings small, 768 for nomic-embed-text with Ollama, or select another dimension based on your model."
    )
        ...
        with col2:
            if table_has_data:
                ...
                if st.button("Clear Table Data"):
                    try:
                        with st.spinner("Scheduling table clearing job..."):
                            # Call the NEW scheduler function instead of the direct truncate function
                            response = supabase.rpc(
                                'schedule_truncate_site_pages_job').execute()

                            # Check response data if needed, assuming success if no exception
                            st.success(
                                "✅ Table clearing job scheduled successfully! It may take a minute or two to complete.")
                            # Consider removing st.rerun() as the table isn't empty yet.
                            # Maybe add a "Refresh Status" button or just let the user know.
                    except Exception as e:
                        st.error(
                            f"Error scheduling table clearing job: {str(e)}")
                        # Keep the fallback manual instructions as a last resort
                        st.info(
                            "This might happen if the scheduling function or pg_cron is not set up correctly, or permissions are missing.")
                        truncate_sql = "TRUNCATE TABLE public.site_pages;"
                        st.code(truncate_sql, language="sql")
                        st.info(
                            "As a fallback, execute this SQL in your Supabase SQL Editor.")
                        supabase_url = get_env_var("SUPABASE_URL")
                        if supabase_url:
