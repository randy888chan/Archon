...
def environment_tab():
    ...
    with col1:
        ...
        selected_profile = st.selectbox(
            ...
            index=profiles.index(
                current_profile) if current_profile in profiles else 0,
        ...
        if selected_profile != current_profile:
            if set_current_profile(selected_profile):
                ...
                st.success(
                    f"Switched to profile: {selected_profile}, reloading...")
    ...
    with col2:
        ...
        new_profile_name = st.chat_input(
            "New Profile Name", key="new_profile_name")
    ...
    if selected_profile != "default" and selected_profile == current_profile:
        if st.button("Delete Current Profile", key="delete_profile"):
            if delete_profile(selected_profile):
                ...
                st.success(
                    f"Deleted profile: {selected_profile}, reloading...")
                reload_archon_graph(show_reload_success=False)
                st.rerun()
            else:
    ...
    if "llm_provider" not in st.session_state:
        st.session_state.llm_provider = profile_env_vars.get(
            "LLM_PROVIDER", "OpenAI")

    if "embedding_provider" not in st.session_state:
        st.session_state.embedding_provider = profile_env_vars.get(
    ...
    selected_llm_provider = st.selectbox(
        ...
        index=llm_providers.index(
            st.session_state.llm_provider) if st.session_state.llm_provider in llm_providers else 0,
    ...
    selected_embedding_provider = st.selectbox(
        ...
        index=embedding_providers.index(
            st.session_state.embedding_provider) if st.session_state.embedding_provider in embedding_providers else 0,
    ...
    with st.form("env_vars_form"):
        ...
        current_embedding_base_url = profile_env_vars.get(
            "EMBEDDING_BASE_URL", embedding_default_urls.get(selected_embedding_provider, ""))
        ...
        if not current_embedding_base_url or profile_env_vars.get("EMBEDDING_PROVIDER", "") != selected_embedding_provider:
            current_embedding_base_url = embedding_default_urls.get(
        ...
        current_embedding_api_key = profile_env_vars.get(
            "EMBEDDING_API_KEY", "")
        ...
        # If there's already a value, show asterisks in the placeholder
        placeholder = "Set but hidden" if profile_env_vars.get(
            "SUPABASE_SERVICE_KEY", "") else ""
        ...
        # 4. Pinecone Section
        st.header("4. Pinecone Vector Database")

        # PINECONE_API_KEY
        pinecone_api_key_help = "Your Pinecone API key - Get this from your Pinecone dashboard"

        # If there's already a value, show asterisks in the placeholder
        placeholder = "Set but hidden" if profile_env_vars.get(
            "PINECONE_API_KEY", "") else ""
        pinecone_api_key = st.text_input(
            "PINECONE_API_KEY:",
            type="password",
            help=pinecone_api_key_help,
            key="input_PINECONE_API_KEY",
            placeholder=placeholder
        )
        # Only update if user entered something (to avoid overwriting with empty string)
        if pinecone_api_key:
            updated_values["PINECONE_API_KEY"] = pinecone_api_key

        # PINECONE_INDEX_NAME
        pinecone_index_name_help = "Name of your Pinecone index"

        pinecone_index_name = st.text_input(
            "PINECONE_INDEX_NAME:",
            value=profile_env_vars.get("PINECONE_INDEX_NAME", ""),
            help=pinecone_index_name_help,
            key="input_PINECONE_INDEX_NAME"
        )
        updated_values["PINECONE_INDEX_NAME"] = pinecone_index_name

        # PINECONE_CLOUD
        pinecone_cloud_help = "Cloud provider for Pinecone (e.g., aws, gcp, azure)"

        pinecone_cloud = st.text_input(
            "PINECONE_CLOUD:",
            value=profile_env_vars.get("PINECONE_CLOUD", ""),
            help=pinecone_cloud_help,
            key="input_PINECONE_CLOUD"
        )
        updated_values["PINECONE_CLOUD"] = pinecone_cloud

        # PINECONE_REGION
        pinecone_region_help = "Region for your Pinecone deployment (e.g., us-east-1, us-west-2, etc.)"

        pinecone_region = st.text_input(
            "PINECONE_REGION:",
            value=profile_env_vars.get("PINECONE_REGION", ""),
            help=pinecone_region_help,
            key="input_PINECONE_REGION"
        )
        updated_values["PINECONE_REGION"] = pinecone_region
        ...
        if submitted:
            ...
            for var_name, value in updated_values.items():
                # Allow empty strings for API keys (they might be intentionally cleared)
                if value or var_name in ["LLM_API_KEY", "EMBEDDING_API_KEY"]:
            ...
            if success:
                st.success(
                    f"Environment variables saved successfully to profile: {current_profile}!")
                reload_archon_graph()
